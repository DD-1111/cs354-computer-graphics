...
glm::dvec3 RayTracer::traceRay(ray& r, const glm::dvec3& thresh, int depth, double& t )
{
	...
	if(scene->intersect(r, i)) {
		...
		double t = i.getT();
		glm::dvec3 Q = P + d*t;
		ray reflected_ray(glm::dvec3(0,0,0), glm::dvec3(0,0,0), glm::dvec3(1,1,1), ray::REFLECTION);
		reflected_ray.setPosition(Q);
		glm::dvec3 reflection_dir = glm::reflect(d, N);
		reflection_dir = glm::normalize(reflection_dir);
		reflected_ray.setDirection(reflection_dir);
		if(depth > 0){
			glm::dvec3 sub_rays_color = traceRay(reflected_ray, thresh, depth - 1, t);
			colorC = colorC + m.kr(i) * sub_rays_color;
		}
		double n_i;
		double n_j;
		double in_out = glm::dot(d,N);
		if(in_out < 0){
			n_i = 1.0;
			n_j = m.index(i);
		}else{
		...
		bool not_zero = m.kt(i).x > 0 || m.kt(i).y > 0 || m.kt(i).z > 0;
		ray refraction_ray(glm::dvec3(0,0,0), glm::dvec3(0,0,0), glm::dvec3(1,1,1), ray::REFRACTION);
		glm::dvec3 v = d * -1.0;
		// check TIR
		bool TIR = pow(n_i/n_j,2)*(1.0 - pow(glm::dot(N,v),2)) <= 1;

		if(not_zero && TIR){
			refraction_ray.setPosition(Q);
			refraction_ray.setDirection(glm::refract(d,N, n_n));
			if(depth > 0){
				colorC = colorC + m.kt(i) * traceRay(refraction_ray, thresh, depth - 1, t);
			}
		}
	} else {
